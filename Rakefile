  ...
  def generate_package_manifest
    ...
    specs_with_executables.each do |spec|
      spec.executables.each do |executable|
        ...
        manifest_line = "#{gem_repo_dir}/#{executable_path} store=%INSTALLDIR%/lib/ruby/gems/1.9.1/#{executable_path} mode=555 program_id=#{((program_id_index += 1)%31)+1}"
        ...
        manifest_line = "#{gem_repo_dir}/#{executable_path} store=%INSTALLDIR%/bin/#{executable} mode=555 program_id=#{((program_id_index += 1)%31)+1}"
        manifest_lines << manifest_line
      end
