...
#define GC_MALLOC_LIMIT 200000000
#endif
...
ruby_memerror(void)
{
	...
	if (ruby_native_thread_p()) {
	    ...
	    /* no ruby thread */
	    fprintf(stderr, "[FATAL] ruby_memerror failed to allocate memory\n");
	    exit(EXIT_FAILURE);
	}
    }
}
...
rb_memerror(void)
{
	...
	fprintf(stderr, "[FATAL] rb_memerror failed to allocate memory\n");
	exit(EXIT_FAILURE);
    }
...
garbage_collect_with_gvl(rb_objspace_t *objspace)
{
	...
	if (ruby_native_thread_p()) {
	    ...
	    /* no ruby thread */
	    fprintf(stderr, "[FATAL] GC failed to allocate memory\n");
	    exit(EXIT_FAILURE);
	}
    }
}
...
Init_GC(void)
{
    ...
    nomem_error = rb_exc_new3(rb_eNoMemError,
			      rb_obj_freeze(rb_str_new2("new3 failed to allocate memory")));
